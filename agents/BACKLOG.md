...
| A-014 | P2 | Generic gateway route registry — domains register routes, not hard-coded in server | Draft | TBD | the one remaining domain-coupling (string dispatch in gateway.zig route()); needs a route-table + per-route param extraction. Flagged in REVIEW-012 |
