...
OUT_DIR="${ROOT_DIR}/profiles/$(date +%Y%m%d-%H%M%S)-client"
PERF_DATA="${OUT_DIR}/perf-client.data"
PERF_SCRIPT="${OUT_DIR}/perf-client.script"
FOLDED="${OUT_DIR}/perf-client.folded"
SVG="${OUT_DIR}/client-flamegraph.svg"
...
./zig-out/bin/wormdb >"${SERVER_LOG}" 2>&1 &
SERVER_PID=$!
...
# Wait until WormDB is accepting TCP connections before profiling client workload.
ready=0
...
perf record -F 199 -g --call-graph dwarf,16384 -o "${PERF_DATA}" -- \
  bun run bench \
    --tests "${BENCH_TESTS}" \
    --ops "${BENCH_OPS}" \
...
"${ROOT_DIR}/.tools/FlameGraph/flamegraph.pl" --title "WormDB Bun Client CPU Flamegraph" "${FOLDED}" > "${SVG}"

popd >/dev/null

echo "Client flamegraph generated: ${SVG}"
