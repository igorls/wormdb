...
//!   mem_get          <ns> <doc_id>
//!   mem_query        <ns> <embedding> <k> [<lambda>] [<min_score>] [<snippet_chars>] [<filter>]
//!   mem_stats        <ns>
//!   mem_drop         <ns>
...
fn runFilteredBruteScan(
    ...
    for (results) |r| {
        ...
        const raw_sim = computeExactSim(metric, query_vec, vec);
        const score = applyDecay(raw_sim, r.timestamp, decay, now_ms);
...
pub fn memQuery(ctx: *Ctx) anyerror!Ctx.Result {
    ...
    if (ctx.arg(6)) |raw_filter| {
        parsed_filter = predicate.parse(ctx.allocator, raw_filter) catch
    ...
    if (!used_hnsw) {
        if (filter_ptr) |filter| {
            try runFilteredBruteScan(ctx, ns, vec_namespace, query_vec, metric, decay, now_ms, filter, &final_heap);
        } else {
